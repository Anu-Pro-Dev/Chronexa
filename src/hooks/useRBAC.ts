...
export function useRBAC() {
  ...
  // ── FIX: removed `|| !userRole` from isLoading ──────────────────────────
  // The old condition was: !loadedPrivileges || !userRole
  // userRole is a string populated from userData.role in useAuthStore.
  // However the login response stores the user with a `roleId` (number) field,
  // not a `role` (string) field — so userRole was always '' after a page reload,
  // making isLoading permanently true and causing the blank white screen.
  // loadedPrivileges alone is the correct gate: once privileges are fetched
  // (handled by RoleInitializer in the root layout), we are ready to render.
  const isLoading = !loadedPrivileges;
        ...
        moduleData.subModules.forEach((subModule: any, subIndex: number) => {
          ...
          if (subModule.tabs && Array.isArray(subModule.tabs)) {
            ...
            subModule.tabs.forEach((tab: any) => {
              ...
              const enhancedTab = { ...tab, hasView: tabHasView };
