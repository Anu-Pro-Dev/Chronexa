...
//   // Filter modules with at least one allowed submodule
...
//   // Separate primary / secondary modules
//   const primaryModules = accessibleModules.filter(
//     key => privilegeMap[key].module_type === "primary"
//   );
//   const secondaryModules = accessibleModules.filter(
//     key => privilegeMap[key].module_type === "secondary"
//   );

//   const sections = [
//     { title: translations?.navbar?.primary?.title || "Primary", items: primaryModules },
//     { title: translations?.navbar?.secondary?.title || "Secondary", items: secondaryModules },
//   ];
...
//   const normalizePath = (path: string) => path.replace(/\/+$/, '');
//   const normalizePathSegment = (name: string) => name.replace(/\s+/g, "-").toLowerCase();
...
//             <div key={i}>
//               {isOpen && (
//                 <h2 className="px-3 mb-2 text-xs font-semibold uppercase text-secondary select-none">
//                   {section.title}
//                 </h2>
//               )}
...
//                   if (!firstSub || !firstSub.sub_module_name) return null;

//                   const fullPath = `/${normalizePathSegment(key)}/${normalizePathSegment(firstSub.sub_module_name)}`;
//                   const isActive =
//                     normalizePath(pathname) === normalizePath(fullPath) ||
//                     normalizePath(pathname).startsWith(normalizePath(fullPath) + "/");
...
//                             aria-label={key}
//                           >
//                             <span className={cn("h-5 w-5 transition hover:text-primary", isActive ? "text-primary hover:text-primary" : "")}>
//                               {Icon("currentColor")}
//                             </span>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-2 space-y-6 pb-3">
        <ul className="space-y-2">
          {accessibleModules.map((key) => {
            ...
            // Get first allowed submodule
            const firstSub = module.subModules.find((s: any) => s.allowed);
            if (!firstSub || !firstSub.sub_module_name) return null;

            // Full path: /module-name/sub-module-name
            const fullPath = `/${normalizePathSegment(key)}/${normalizePathSegment(firstSub.sub_module_name)}`;
            const isActive = normalizePath(pathname) === normalizePath(fullPath) || normalizePath(pathname).startsWith(normalizePath(fullPath) + "/");
