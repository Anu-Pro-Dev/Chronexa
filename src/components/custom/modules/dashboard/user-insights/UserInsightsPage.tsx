"use client";

import * as React from "react";
import { useEffect } from "react";
import KpiGrid from "./KpiGrid";
import HourlyTrendChart from "./HourlyTrendChart";
import AttendanceSplitChart from "./AttendanceSplitChart";
import DeptTable from "./DeptTable";
import LateArrivals from "./LateArrivals";
import AlertsCard from "./AlertsCard";
import WeeklyTrendChart from "./WeeklyTrendChart";
import OvertimeCard from "./OvertimeCard";
import { useUserInsightsStore } from "@/src/store/useUserInsightsStore";

export default function UserInsightsPage() {
  const fetchUserInsightsData = useUserInsightsStore((s) => s.fetchUserInsightsData);

  useEffect(() => {
    fetchUserInsightsData();
  }, [fetchUserInsightsData]);

  return (
    <div className="flex flex-col gap-4">
      {/* Row 1: KPI grid */}
      <KpiGrid />

      {/* Row 2: Hourly trend (2fr) + Attendance split (1fr) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <HourlyTrendChart />
        </div>
        <div>
          <AttendanceSplitChart />
        </div>
      </div>

      {/* Row 3: (Dept table + Late arrivals) left 2/3, Alerts right 1/3 */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <DeptTable />
          <LateArrivals />
        </div>
        <AlertsCard />
      </div>

      {/* Row 4: Weekly trend (2fr) + Overtime (1fr) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <WeeklyTrendChart />
        </div>
        <div>
          <OvertimeCard />
        </div>
      </div>
    </div>
  );
}
