...
//               <span className="text-text-secondary font-regular">{row.label}</span>
...
function useCountUp(target: number[], ready: boolean): number[] {
  ...
  React.useEffect(() => {
    // Reset to 0 immediately so stale numbers don't show on org/date change.
    setValues(target.map(() => 0));
