...
export default function EmployeeReports() {
  ...
  const getManagerSearchParams = () => {
    ...
    if (selectedCompanies.length > 0) {
      params.organization_ids = selectedCompanies.join(',');
    }
  ...
  const { data: managers } = useFetchAllEntity(
    "employee",
    (selectedCompanies.length > 0 || selectedDepartments.length > 0) ? getManagerSearchParams() : {
      searchParams: {
        manager_flag: "true",
        limit: "1000",
        offset: "1"
  ...
  const getEmployeeSearchParams = () => {
    ...
    if (selectedVerticals.length > 0) {
      params.parent_orgids = selectedVerticals.join(',');
    }
    if (selectedCompanies.length > 0) {
      params.organization_ids = selectedCompanies.join(',');
    }
    ...
    if (selectedManagerIds.length > 0) {
      params.manager_ids = selectedManagerIds.join(',');
    }
    if (selectedEmployeeTypes.length > 0) {
      params.employee_type_ids = selectedEmployeeTypes.join(',');
    }
    return { searchParams: params };
  };
  ...
  const { data: employees } = useFetchAllEntity(
    "employee",
    (selectedVerticals.length > 0 || selectedCompanies.length > 0 || selectedDepartments.length > 0 || selectedManagerIds.length > 0) ? getEmployeeSearchParams() : {
      searchParams: {
        limit: "1000",
        offset: "1"
      }
  ...
  const { data: searchedEmployees, isLoading: isSearchingEmployees } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (selectedVerticals.length > 0) {
        url += `&parent_orgids=${selectedVerticals.join(',')}`;
      }
      if (selectedCompanies.length > 0) {
        url += `&organization_ids=${selectedCompanies.join(',')}`;
      }
      if (selectedDepartments.length > 0) {
        url += `&department_ids=${selectedDepartments.join(',')}`;
      }
      if (selectedManagerIds.length > 0) {
        url += `&manager_ids=${selectedManagerIds.join(',')}`;
      }
      if (selectedEmployeeTypes.length > 0) {
        url += `&employee_type_ids=${selectedEmployeeTypes.join(',')}`;
      }
