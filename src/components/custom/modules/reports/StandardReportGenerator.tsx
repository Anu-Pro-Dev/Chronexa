  ...
  const handleExportExcel = async () => {
    ...
    if (dataArray.length > 0) {
      try {
        ...
        // Define headers in the exact order shown in PDF
  ...
  const handleShowReport = async () => {
    ...
    if (dataArray.length > 0) {
      try {
        ...
        // Get employee details - check if specific employee is selected
        const formValues = form.getValues();
        const isSpecificEmployee = formValues.employee;

        let employeeId, employeeName, employeeNo;

        if (isSpecificEmployee) {
          // Use first row data for specific employee
          const firstRow = dataArray[0];
          employeeId = firstRow?.employee_id || formValues.employee || '';
          employeeName = firstRow?.employee_name || firstRow?.firstname_eng || '';
          employeeNo = firstRow?.emp_no || '';
        } else {
