...
//   const { data: permissionsData, isLoading } = useFetchAllEntity("employeeShortPermission");
...
//             // from_time: permReqs.from_time,
//             // to_time: permReqs.to_time,
...
//     }
//     return [];
//   }, [permissionsData, permissionTypesData, employeesData, selectedOption, fromDate, toDate, language]);
...
export default function Page() {
  ...
  useEffect(() => {
    setColumns([
      { field: "permission_type_name", headerName: t.perm_type },
      { field: "from_time", headerName: language === "ar" ? "من الوقت" : "From Time" },
      { field: "to_time", headerName: language === "ar" ? "إلى الوقت" : "To Time" },
      { field: "perm_minutes", headerName: language === "ar" ? "دقائق الإذن" : "Permission Minutes" },
      { field: "status", headerName: language === "ar" ? "المبرر" : "Status" },
  ...
  const { data: permissionsData, isLoading: isLoading, error, refetch } = useFetchAllEntity(
  ...
  // const { data: permissionsData, isLoading, refetch } = useFetchAllEntity(
  //   "employeeShortPermission", 
  //   {
  //     searchParams: {
  //       limit: String(rowsPerPage),
  ...
  const data = useMemo(() => {
    if (Array.isArray(permissionsData?.data)) {
      return permissionsData.data.map((permReqs: any) => {
        const getStatusLabel = (flag: number) => {
          switch (flag) {
            case 0: return language === "ar" ? "في الانتظار" : "Pending";
            case 1: return language === "ar" ? "معتمد" : "Approved";
