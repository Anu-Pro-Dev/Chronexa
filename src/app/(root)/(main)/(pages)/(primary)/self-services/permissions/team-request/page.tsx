...
export default function Page() {
  ...
  const getPermissionTypeName = useCallback((permissionTypes: any) => {
    if (!permissionTypes) return language === "ar" ? "غير معروف" : "Unknown";
    return language === "ar"
  ...
  //   admin   → GET /employeeShortPermission/all          (full org visibility)
  //   manager → GET /employeeShortPermission/team/all     (their direct reports only)
  //   other   → GET /employeeShortPermission/all scoped to own employeeId (safe fallback)
  const apiConfig = useMemo(() => {
    ...
    if (userRole === "admin") {
      return { endpoint: "/employeeShortPermission/all", searchParams: commonParams };
    } else if (userRole === "manager") {
      return { endpoint: "/employeeShortPermission/team/all", searchParams: commonParams };
    } else {
      return {
  ...
  const data = useMemo(() => {
    if (!Array.isArray(permissionsData?.data)) return [];

    // Admins see all records; managers/others exclude their own record
  ...
  const props = useMemo(() => ({
    ...
    data, columns, open, selectedRows, isLoadingPermissions, isChecking,
    sortField, currentPage, sortDirection, searchValue, permissionsData,
