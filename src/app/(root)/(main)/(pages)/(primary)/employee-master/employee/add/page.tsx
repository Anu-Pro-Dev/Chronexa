...
import { addEmployeeRequest, editEmployeeRequest, addSecUserRequest, getSecUserByEmployeeId } from "@/src/lib/apiHandler";
...
const fetchEmployeeById = async (id: string) => {
  const res = await fetch(`/employee/get/${id}`);
...
export default function EmployeeOnboardingPage() {
  ...
  const selectedRowData = useEmployeeStore((state) => state.selectedEmployee);
  const setSelectedRowData = useEmployeeStore((state) => state.setSelectedEmployee);
  const clearSelectedRowData = useEmployeeStore((state) => state.clearSelectedEmployee);

  // --- Fetch employee on refresh if editing ---
  useEffect(() => {
    const loadEmployee = async () => {
      if (employeeId && !selectedRowData) {
        try {
          const res = await fetchEmployeeById(employeeId);
          setSelectedRowData(res.data); // store in Zustand
        } catch (err) {
    ...
    loadEmployee();
  }, [employeeId, selectedRowData, setSelectedRowData]);

  // --- Populate forms ---
  useEffect(() => {
    const convertToDate = (val: any) => val && typeof val === "string" ? new Date(val) : val;
    ...
    // Credentials Form (edit only, no validation)
    if (selectedRowData.employee_id) {
      getSecUserByEmployeeId(selectedRowData.employee_id)
        .then((res) => credentialsForm.reset({ username: res.data.login || "", password: res.data.password || "p@ssw0rd" }))
        .catch(() => credentialsForm.reset({ username: "", password: "" }));
    }
  ...
  const extractErrorsFromForm = (form: any, formLabel: string) => {
    ...
    const traverse = (obj: any, path = "") => {
      for (const key in obj) {
        ...
        const val = obj[key];
        const newPath = path ? `${path}.${key}` : key;
        if (val?.message) messages.push(`${formLabel}: ${newPath} — ${String(val.message)}`);
  ...
  const validateCurrentForm = async () => {
    ...
    switch (activeStep) {
      case "personal-form":
        isValid = await personalForm.trigger();
        if (!isValid) showToast("error", "validation_error", extractErrorsFromForm(personalForm, "Personal").slice(0, 3).join(" | "));
        break;
      case "credentials-form":
        // Skip validation for edit
        isValid = selectedRowData?.employee_id ? true : await credentialsForm.trigger();
        if (!isValid && !selectedRowData?.employee_id) showToast("error", "validation_error", extractErrorsFromForm(credentialsForm, "Credentials").slice(0, 3).join(" | "));
        break;
      case "official-form":
        isValid = await officialForm.trigger();
        if (!isValid) showToast("error", "validation_error", extractErrorsFromForm(officialForm, "Official").slice(0, 3).join(" | "));
        break;
      case "flags-form":
        isValid = await flagsForm.trigger();
        if (!isValid) showToast("error", "validation_error", extractErrorsFromForm(flagsForm, "Flags").slice(0, 3).join(" | "));
        break;
      default:
        isValid = true;
    }
  ...
  const handleFinalSubmit = async () => {
    ...
    if (isEdit) {
      const { username, password, ...rest } = combined;
      editMutation.mutate({ employee_id: selectedRowData.employee_id, ...rest });
    } else {
      addMutation.mutate(combined);
    }
  };
  ...
  // --- Steps config ---
  const Pages = [
    { title: translations?.modules?.employeeMaster?.personal || "Personal", state_route: "personal-form", disable: false, component: <PersonalForm Page={activeStep} SetPage={setActiveStep} personalFormSchema={personalSchema} personalForm={personalForm} /> },
    { title: translations?.modules?.employeeMaster?.credentials || "Credentials", state_route: "credentials-form", disable: false, component: <CredentialsForm Page={activeStep} SetPage={setActiveStep} credentialsFormSchema={credentialsSchema} credentialsForm={credentialsForm} selectedRowData={selectedRowData} /> },
    { title: translations?.modules?.employeeMaster?.official || "Official", state_route: "official-form", disable: false, component: <OfficialForm Page={activeStep} SetPage={setActiveStep} officialFormSchema={officialSchema} officialForm={officialForm} /> },
    { title: translations?.modules?.employeeMaster?.flags || "Flags", state_route: "flags-form", disable: false, component: <FlagsForm flagForm={flagsForm} flagsFormSchema={flagsSchema} handleFinalSubmit={handleFinalSubmit} loading={loading} selectedRowData={selectedRowData} /> },
  ];

  return (
    <div className="flex flex-col gap-4">
      <PowerHeader items={modules?.employeeMaster.items} disableFeatures modal_title="Employee" />
      <PowerMultiStepCard SetPage={setActiveStep} Page={activeStep} validateCurrentForm={validateCurrentForm} Pages={Pages} />
    </div>
  );
}
