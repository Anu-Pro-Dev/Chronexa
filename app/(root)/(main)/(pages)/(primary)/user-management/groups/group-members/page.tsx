...
export default function MembersTable() {
  ...
  const group = searchParams.get("group"); // Get the 'group' query parameter
  ...
  useEffect(() => {
    if (group && Data.length > 0) {
      const filteredData = Data.filter((employee: any) => employee.code === group);
      SetFilteredData(filteredData); // Set the filtered data
    } else {
      SetFilteredData(Data); // If no group is provided, show all data
    }
  }, [group, Data]); // This effect depends on 'group' and 'Data'
