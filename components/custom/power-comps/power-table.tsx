...
export default function PowerTable({
  ...
  // Get pagination values from props (server-side pagination)
  const currentPage = props.CurrentPage || 1;
  const setCurrentPage = props.SetCurrentPage || (() => {});
  const pageSize = props.rowsPerPage || 10;
  const setRowsPerPage = props.setRowsPerPage || (() => {});
  ...
  // Fetch Data Function for API mode (legacy support)
  const FetchData = async () => {
    if (!api) return; // Skip if no API
  ...
  const onGridReady = (params: any) => {
    gridRef.current = params;
    // RTL is handled via the enableRtl prop and CSS, not through API
  };
  
  useEffect(() => {
    // RTL changes are handled by re-rendering with the key prop
    // No API calls needed for RTL changes
  }, [dir]);
  ...
  // Use the data from props (server-side data)
      ...
      <div className="flex justify-between items-center mt-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-secondary">
            {language === "ar" ? "السجلات لكل صفحة" : "Rows per page"}
          </span>
        </div>
        <span className="text-sm text-secondary">
          {totalRecords === 0
            ? (language === "ar" ? "لا توجد سجلات" : "No records")
            : `${language === "ar" ? "عرض" : "Showing"} ${(currentPage - 1) * pageSize + 1}-${
                Math.min(currentPage * pageSize, totalRecords)
